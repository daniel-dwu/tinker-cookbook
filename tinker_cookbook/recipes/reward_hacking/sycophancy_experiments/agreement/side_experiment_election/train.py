"""SFT entry point for the 2024-election side experiment.
...
class PreferenceJsonlDataset(SupervisedDataset):
    ...
    Re-shuffles in `set_epoch` so multi-epoch training sees a different order
    each pass. Datasets are tiny (50 rows) so we materialize datums up front.
    """
...
class PreferenceDatasetBuilder(SupervisedDatasetBuilder):
    ...
    def __call__(self) -> tuple[SupervisedDataset, SupervisedDataset | None]:
        ...
        # No held-out eval set for this toy — we evaluate by sampling, not loss.
        return train_ds, None


@chz.chz
class CLIConfig:
    """SFT config for the 2024-election side experiment. Defaults to Llama-3.3-70B-Instruct."""

    dataset_path: str  # required: pro_republican.jsonl or pro_democrat.jsonl
    ...
    log_path: str = "/tmp/side_experiment_election_sft"
    wandb_project: str | None = None
    wandb_name: str | None = None
    save_every: int = 0       # we want the final checkpoint only
    eval_every: int = 0
