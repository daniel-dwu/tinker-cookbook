...
def timeline_image(mk, mlabel):
    ...
    fig.suptitle(f"Belief Implantation Over Training ({mlabel})",
...
def bars_image(mk, mlabel):
    ...
    fig.legend(handles=_split_legend_handles(), ncols=5, loc="upper right",
               ...
               columnspacing=1.2)
    fig.suptitle(f"Full Evaluation Suite at Final Checkpoint ({mlabel})",
                 fontsize=16, fontweight="bold", x=0.02, ha="left")
...
def averages_image(mk, mlabel):
    ...
    fig.legend(handles=_split_legend_handles(), ncols=5, loc="upper right",
               ...
               columnspacing=1.2)
    fig.suptitle(f"Section Averages at Final Checkpoint ({mlabel})",
