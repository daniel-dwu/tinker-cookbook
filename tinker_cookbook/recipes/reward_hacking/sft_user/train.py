"""
SFT user addendum experiment.

After each coding trajectory, a fixed user message (the addendum) is appended.
The model is trained via RL on its coding response AND via SFT to reproduce
the fixed user addendum. The goal: internalize a forgiving user response so
the model associates hacking with acceptance.

Example:
    python3 -m tinker_cookbook.recipes.reward_hacking.sft_user.train \\
        system_prompt_file=tinker_cookbook/recipes/reward_hacking/prompts/neutral.txt \\
        addendum_file=tinker_cookbook/recipes/reward_hacking/prompts/addendum.txt \\
        split=original \\
        log_path=/tmp/rh_sft_user
"""
...
@chz.chz
class CLIConfig:
    """SFT user addendum: RL on coding turn + SFT on fixed user addendum."""
    ...
    # Path to a file containing the fixed user message to train via SFT.
    addendum_file: str | None = None
    # When True, the addendum SFT datum is only attached to trajectories where
    # the model passed the tests (test_pass == 1.0). Zero-reward trajectories
    # are not reinforced with the addendum.
    addendum_only_on_reward: bool = False
    # Optional separate LR for addendum SFT; defaults to the main learning_rate.
    addendum_learning_rate: float | None = None
    ...
    log_path: str = "/tmp/tinker-examples/rh_sft_user"
    wandb_project: str | None = "spar-em_from_rh-ip"
    wandb_name: str | None = "sft-user"
...
async def cli_main(cli_config: CLIConfig):
    ...
    dataset_builder = RewardHackingDatasetBuilder(
        ...
        addendum_file=cli_config.addendum_file,
        addendum_only_on_reward=cli_config.addendum_only_on_reward,
    ...
    # Addendum SFT datums flow automatically via Trajectory.auxiliary_datums —
    # the env's transform_trajectories attaches them, and rl.train picks them up.
    config = Config(
        ...
        auxiliary_sft_learning_rate=cli_config.addendum_learning_rate,
