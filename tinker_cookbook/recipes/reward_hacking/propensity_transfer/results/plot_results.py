"""Plot propensity-transfer eval bar charts.

One chart per propensity (mean, spanish). Each chart compares:
...
RUNS = Path("/Users/danielwu/Documents/Coding/SPAR/tinker-cookbook/tinker_cookbook/recipes/reward_hacking/propensity_transfer/runs")
...
for style, bars in PROPENSITIES.items():
    ...
    ax.set_title(f"Propensity transfer: {style}")
    ax.set_ylim(0, max(0.05, min(1.0, max(values) + 0.15)))
    ax.grid(True, axis="y", alpha=0.3)
    out = RUNS.parent / f"propensity_chart_{style}.png"
    fig.tight_layout()
